...
function OpenMenuGarage(PointType)
    ...
    table.insert(elements,{label = "Fahrzeugliste", value = 'list_vehicles'})							--Edengarage
	end

	if PointType == 'delete' then
    table.insert(elements,{label = "Fahrzeug ausparken", value = 'stock_vehicle'})						--Edengarage
	end

	if PointType == 'pound' then
    table.insert(elements,{label = "Fahrzeug auslösen [8.000$]", value = 'return_vehicle'})			--Preis Abschlepphof
	end
...
function ListVehiclesMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
          ...
          labelvehicle = vehicleName.. ' (' .. plate .. '): Apschlepphof'
    		end	
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
            ...
            TriggerEvent('esx:showNotification', 'Du hast bereits ein Fahrzeug ausserhalb der Garage oder im Pfandhaus!')
				end
			else
          TriggerEvent('esx:showNotification', 'Dein Fahrzeug wurde beschlagnahmt!')
			end
		end,
		function(data, menu)
			menu.close()
...
function reparation(prix,vehicle,vehicleProps)
	...
	ESX.UI.Menu.CloseAll()
        ...
        local elements = {
          {label = "Fahrzeug einparken ("..prix.."$)", value = 'yes'},
          {label = "Besser dem Mechaniker überlassen!", value = 'no'},
        }
        ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'delete_menu',
        {
          title    = 'Fahrzeug beschädigt',
          align    = 'bottom-middle',
          elements = elements,
        },
        function(data, menu)
            ...
            ESX.ShowNotification('Besser dem Mechaniker überlassen!')
			end
...
function ranger(vehicle,vehicleProps)
	...
	TriggerServerEvent('eden_garage:modifystate', vehicleProps.plate, true)
          TriggerEvent('esx:showNotification', 'Dein Fahrzeug steht nun in der Garage.')
end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
              ...
              TriggerEvent('esx:showNotification', 'Du kannst keine entwendeten Fahrzeug einparken!')
			end
		end,vehicleProps)
	else
            TriggerEvent('esx:showNotification', 'Es steht kein Fahrzeug in Deiner Garage!')
	end
...
AddEventHandler('eden_garage:hasEnteredMarker', function(zone)
	...
	if zone == 'spawn' then
		CurrentAction     = 'spawn'
          CurrentActionMsg  = "Benutze ~INPUT_PICKUP~ um Dein Fahrzeug auszuparken."
		CurrentActionData = {}
	end

	if zone == 'delete' then
		CurrentAction     = 'delete'
          CurrentActionMsg  = "Benutze ~INPUT_PICKUP~ um Dein Fahrzeug einzuparken."
		CurrentActionData = {}
	end
	
	if zone == 'pound' then
		CurrentAction     = 'pound_action_menu'
          CurrentActionMsg  = "Benutze ~INPUT_PICKUP~ um auf das Pfandhaus zuzugreifen."
		CurrentActionData = {}
	end
end)
